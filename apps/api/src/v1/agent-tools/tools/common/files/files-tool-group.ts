...
export class FilesToolGroup extends BaseToolGroup<FilesToolGroupConfig> {
  ...
  public getDetailedInstructions(
    ...
    const lines: string[] = [
      ...
      '1. codebase_search — semantic search, returns absolute file paths, code snippets, and total_lines per file. FIRST STEP for any "where is X?" question.',
      ...
      '- codebase_search returns `total_lines` for each result. Use it to decide your read strategy.',
      '- Small files (≤300 lines): read entirely with files_read.',
      '- Large files (>300 lines): NEVER read the full file. Use fromLineNumber/toLineNumber in files_read to fetch only the relevant section (start_line/end_line from codebase_search ± padding). Reading full large files wastes context and slows analysis.',
      '',
