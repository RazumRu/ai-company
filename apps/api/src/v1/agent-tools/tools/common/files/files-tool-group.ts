...
export class FilesToolGroup extends BaseToolGroup<FilesToolGroupConfig> {
  ...
  public getDetailedInstructions(
    ...
    const lines: string[] = [
      ...
      '',
      '⚠️ PREFERRED WORKFLOW — follow this order:',
      '',
      'STEP 1 — Start with codebase_search:',
      '- After cloning a repo with gh_clone, your FIRST action SHOULD be codebase_search.',
      '- Prefer codebase_search over files_directory_tree or files_find_paths — it is faster, more precise, and wastes less context.',
      ...
      '- **If codebase_search reports indexing is in progress**: do NOT retry it. Instead, use files_directory_tree, files_find_paths, and files_search_text to explore the codebase while the index builds.',
      '',
      ...
      '| Task | Tool | Notes |',
      '|---|---|---|',
      '| **Explore codebase** | **codebase_search** | **Preferred first step — fall back to other tools if indexing is in progress** |',
      ...
      '| Browse directory | files_find_paths | List files by glob pattern, or fallback when codebase_search is indexing |',
      '| Directory overview | files_directory_tree | Structural overview, or fallback when codebase_search is indexing |',
      ...
      '- ❌ gh_clone → files_directory_tree → files_find_paths → files_read (skips codebase_search without reason)',
      ...
      '',
      'files_find_paths is for browsing directories by glob pattern when you need to list files by name/extension. Prefer codebase_search for code discovery. Do not use files_find_paths to "verify" paths from codebase_search (they are already absolute).',
      ...editSection,
    ];
