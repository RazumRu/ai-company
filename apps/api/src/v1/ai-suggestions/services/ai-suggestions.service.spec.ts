...
describe('AiSuggestionsService', () => {
  ...
  >;
  let graphCompiler: Pick<GraphCompiler, 'compileTemporary'>;
    ...
    graphCompiler = {
      compileTemporary: vi
        ...
        .mockImplementation(
          async (
            _entity: GraphEntity,
            callback: (graph: CompiledGraph) => Promise<unknown>,
          ) => callback(buildCompiledGraph()),
        ),
    };
  ...
  describe('suggestGraphInstructions', () => {
    ...
    it('returns same result whether graph is running or not', async () => {
      ...
      const result = await service.suggestGraphInstructions('graph-1', {
