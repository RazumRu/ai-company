...
export class AiSuggestionsService {
  ...
  async analyzeThread(
    ...
    return this.ensureGraphContext(thread.graphId, async (compiledGraph) => {
  ...
  async suggest(
    ...
    return this.ensureGraphContext(graphId, async (compiledGraph) => {
      const graph = await this.graphDao.getOne({
        id: graphId,
  ...
  private async ensureGraphContext<T>(
    graphId: string,
    ...
    const compiled = await this.graphCompiler.compile(graph);

    try {
      return await fn(compiled);
    } finally {
