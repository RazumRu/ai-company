...
// ── Shared utilities (exported for read-only / snapshot views) ────────────────

/** Icon map for node kind avatars — matches the main graph canvas icons. */
export const graphNodeKindIconMap: Record<string, React.ReactNode> = {
...
export const GRAPH_HANDLE_STYLE_TARGET: CSSProperties = {
  width: '12px',
  height: '12px',
  position: 'absolute',
  left: '-18px',
...
/** Handle style for source (output) handles — matches the main graph canvas. */
export const GRAPH_HANDLE_STYLE_SOURCE: CSSProperties = {
  width: '12px',
  height: '12px',
  position: 'absolute',
  right: '-18px',
...
/** Collapsed input handles slot — blue "inputs" badge. Pass Handle component(s) as handleSlot. */
export const NodeCollapsedInputSlot: React.FC<{
  count: number;
...
/** Collapsed output handles slot — green "outputs" badge. Pass Handle component(s) as handleSlot. */
export const NodeCollapsedOutputSlot: React.FC<{
  count: number;
  handleSlot?: React.ReactNode;
}> = ({ count, handleSlot }) => (
...
export interface GraphNodeCardProps {
  ...
  /** Hide the expand/collapse chevron (e.g. in read-only views with no handles). Defaults to true. */
  ...
  ({
    ...
    const agentAvatar = isAgentNode ? (
      <Avatar className="size-6 shrink-0">
        <AvatarFallback className="bg-primary/20 text-primary text-[9px] font-semibold">
          {getAgentInitials(nodeData.label)}
        </AvatarFallback>
      </Avatar>
    ) : null;
    ...
    const avatar = agentAvatar ?? (
      <div className="w-6 h-6 rounded-full bg-primary/15 flex items-center justify-center flex-shrink-0 text-primary">
        {graphNodeKindIconMap[templateKindLower] ?? (
