...
function formatIdleLabel(
  ...
  if (status === RuntimeInstanceDtoStatusEnum.Starting) {
    return 'Starting...';
  }
  if (status === RuntimeInstanceDtoStatusEnum.Stopping) {
    return 'Stopping...';
  }
  ...
  return (
    <HoverCard openDelay={300} closeDelay={100}>
      <HoverCardTrigger asChild>
        <div className="mx-2 my-1.5 px-3 py-2.5 rounded-lg border border-border bg-card hover:bg-muted/40 transition-colors cursor-default">
      ...
      <HoverCardContent side="left" align="start" className="w-80">
        <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 items-baseline text-xs">
          ...
          <span className="text-foreground">{runtime.status}</span>
          {agentName && (
            <>
              <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground/60">
                Agent
              </span>
              <span className="text-foreground break-all">{agentName}</span>
            </>
          )}
          <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground/60">
            Type
          </span>
          <span className="text-foreground">{runtime.type}</span>
          {runtime.containerName && (
            <>
              <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground/60">
                Container
