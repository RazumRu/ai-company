...
export const useChatsThreadList = (deps: UseChatsThreadListDeps) => {
  ...
  useEffect(() => {
    ...
    if (chatIdParam === selectedThreadId) {
      return;
    } // already in sync
    navigate(
      {
  ...
  const loadThreads = useCallback(
    async (options?: { append?: boolean }) => {
      ...
      if (isAppend) {
        if (!threadsHasMoreRef.current || threadsLoadingMoreRef.current) {
          return;
        }
