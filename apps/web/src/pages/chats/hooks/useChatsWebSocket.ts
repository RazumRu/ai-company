...
interface UseChatsWebSocketDeps {
  ...
  setGraphCache: React.Dispatch<
    React.SetStateAction<Record<string, GraphCacheEntry>>
  >;
...
export const useChatsWebSocket = (deps: UseChatsWebSocketDeps) => {
  ...
  const isAgentNodeIdInGraph = useCallback(
    (graphId: string | undefined, nodeId: string | undefined): boolean => {
      ...
      return (graph.agents ?? []).some((a) => a.nodeId === nodeId);
    },
    [graphCache],
  );
    ...
    setGraphCache((prev) => {
      ...
      return {
        ...
        [graphId]: {
          graph: {
            ...existing.graph,
            status: data.data.status as typeof existing.graph.status,
