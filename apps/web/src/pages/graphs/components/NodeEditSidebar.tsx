  ...
  ({
    ...
    const openAiSuggestionModal = useCallback(
      (fieldKey: string, fieldLabel: string, initialValue?: unknown) => {
        if (!isGraphRunning) {
          toastMessage.warning('Start the graph to use AI suggestions');
          return;
        }
    ...
    useEffect(() => {
      ...
      const hydrateForm = async () => {
        ...
        let schema: unknown;
        try {
          const dereferenced = await $RefParser.dereference(rawSchema, {
            mutateInputSchema: false,
          });
          // Flatten allOf structures to help RJSF understand enum fields
          schema =
            dereferenced && typeof dereferenced === 'object'
              ? flattenAllOfInSchema(dereferenced as Record<string, unknown>)
              : dereferenced;
        } catch (error) {
    ...
    const handleAiSuggestionSubmit = useCallback(async () => {
      ...
      if (!userRequest) {
        toastMessage.warning('Enter a request for the AI suggestion');
        return;
      }

      if (!graphId || !node?.id) {
        toastMessage.error('Graph or node is missing for suggestions');
        return;
      }
      ...
      <aside
        ...
        <div
          ...
          <div
            ...
            <span className="text-lg font-semibold">Edit Node</span>
                ...
                {isAgentNode && node?.id && (
                  <Avatar className="size-[30px] shrink-0">
                    <AvatarFallback className="bg-primary/20 text-primary text-xs font-semibold">
                      {getAgentInitials(nodeName || 'Agent')}
                    </AvatarFallback>
                  </Avatar>
                )}
                <span
                  ...
                  {compiledNode?.error && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span>
                          <AlertCircle
                            style={{ color: '#ff4d4f', width: 14, height: 14 }}
                          />
                        </span>
                      </TooltipTrigger>
