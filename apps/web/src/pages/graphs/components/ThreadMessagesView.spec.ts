...
import { computeSubagentRollup } from './threadMessages/threadMessagesViewUtils';
import {
...
describe('computeSubagentRollup', () => {
  ...
  it('strict-partial: returns cost: undefined when some subagents are priced and some are not', () => {
    // Strict-partial policy: any unpriced subagent suppresses the sum entirely
    // to avoid displaying a misleading partial-cost footnote. See plan C-MED-1.
    const items: PreparedMessage[] = [
      makeSubagentWithPrice('s1', 0.045),
  ...
  it('strict-partial: returns cost: undefined when ANY subagent is unpriced', () => {
    // 2 subagents, prices 0.50 + undefined → cost suppressed
    const items: PreparedMessage[] = [
      makeSubagentWithPrice('s1', 0.5),
      makeSubagentNoPrice('s2'),
    ];
    const result = computeSubagentRollup(items);
    expect(result.count).toBe(2);
  ...
  it('strict-partial: returns sum when ALL subagents are priced (regression guard)', () => {
    const items: PreparedMessage[] = [
      makeSubagentWithPrice('s1', 0.5),
      makeSubagentWithPrice('s2', 0.3),
    ];
    const result = computeSubagentRollup(items);
    expect(result.count).toBe(2);
  ...
  it('strict-partial: returns cost: undefined when ALL subagents are unpriced (preserved behavior)', () => {
    const items: PreparedMessage[] = [
      makeSubagentNoPrice('s1'),
      makeSubagentNoPrice('s2'),
    ];
...
// ─── ADVERSARIAL TESTS ───────────────────────────────────────────────────────
// These tests were authored by the adversarial agent to prove currently-unfixed
// bugs. Each test is expected to FAIL on current code.

describe('computeSubagentRollup — adversarial: NaN price poisoning', () => {
  it('NaN totalPrice is typeof "number" so it bypasses strict-partial, poisoning the sum', () => {
    // NaN is typeof 'number', so the current guard `typeof price === 'number'`
    // treats NaN as a priced subagent. NaN + 0.5 = NaN. The result is
    // { count: 2, cost: NaN } rather than { count: 2, cost: undefined }.
    // A NaN cost renders as "$NaN" in the UI — a display bug.
    // This test asserts cost is undefined (the correct behavior), which FAILS today.
    const items: PreparedMessage[] = [
      makeSubagentWithPrice('s1', NaN),
      makeSubagentWithPrice('s2', 0.5),
    ];
    const result = computeSubagentRollup(items);
    // NaN poisons the sum → cost should be treated as unpriced (undefined)
    // Currently fails: result.cost === NaN (not undefined)
    expect(result.cost).toBeUndefined();
  });

  it('single NaN-priced subagent produces cost: NaN instead of cost: undefined', () => {
    // A single subagent with NaN price passes `typeof price === 'number'`
    // and gets added to the cost accumulator. Result: { count: 1, cost: NaN }.
    // The correct behavior would be to treat NaN as unpriced → { count: 1, cost: undefined }.
    const items: PreparedMessage[] = [makeSubagentWithPrice('s1', NaN)];
    const result = computeSubagentRollup(items);
    // Currently fails: result.cost === NaN, not undefined
    expect(result.cost).toBeUndefined();
  });
