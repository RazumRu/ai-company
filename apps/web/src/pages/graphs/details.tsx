...
export const GraphPage = () => {
  ...
  // Clears selection so that reopening the same node triggers fresh hydration
  const handleCloseNodeSidebar = useCallback(() => {
  ...
  const refreshGraphAfterRevision = useCallback(
    async (revision: GraphRevisionDto) => {
      ...
      if (revision.status !== GraphRevisionDtoStatusEnum.Applied) {
        return;
      }
  ...
  const handleEdgesChange = useCallback(
    (changes: Parameters<typeof onEdgesChange>[0]) => {
      ...
      if (isSyncingFromDraftRef.current) {
        return;
      } // Don't update draft if we're syncing FROM draft
  ...
  return (
    <div className="h-full flex overflow-hidden">
      {/* Left: Template sidebar (expanded) or thin collapse strip */}
      {!leftSidebarCollapsed ? (
        <aside className="w-[280px] flex-shrink-0 border-r border-border h-full">
          <TemplateSidebar
        ...
        <div className="w-12 bg-card border-r border-border flex items-start justify-center pt-4 flex-shrink-0">
          <Button
            variant="ghost"
            size="sm"
      ...
      {/* Center: Canvas column with its own header */}
      ...
      <NodeEditSidebar
        ...
        onClose={handleCloseNodeSidebar}
      ...
      <AlertDialog open={confirmResetPending}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Reset all local changes?</AlertDialogTitle>
            <AlertDialogDescription>
              This will discard all unsaved local changes and restore the last
      ...
      <Dialog
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={handleEditGraphClose}
              disabled={isSavingName}>
              Cancel
            </Button>
            <Button
              onClick={() => void handleEditGraphSave()}
      ...
      <AlertDialog open={isDeleteConfirmVisible}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete graph?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete &quot;{graph?.name}&quot; and all its
              revisions. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleDeleteGraphCancel}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDeleteGraphConfirm}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
