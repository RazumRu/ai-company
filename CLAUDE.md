...
**Tech Stack:** React 19 + Vite, Refine framework, Ant Design, TypeScript, Socket.io, @xyflow/react
...
- **Data layer**: Auto-generated API clients from OpenAPI spec (`src/autogenerated/`). Use Refine's `useList`, `useOne`, `useCreate`, etc. for CRUD. Always regenerate after backend changes.
- **State**: Refine handles data fetching/routing/CRUD. `GraphStorageService` provides local persistence for canvas viewport/layout. `WebSocketService` multiplexes graph subscriptions.
...
| Feature | Key files |
|---|---|
| Graph canvas | `src/pages/graphs/components/GraphCanvas.tsx`, `CustomNode.tsx`, `TemplateSidebar.tsx` |
| Node config | `src/pages/graphs/components/NodeEditSidebar.tsx` (JSON-schema driven via `@rjsf/antd`) |
...
| Validation | `src/services/validationService.ts` (template compatibility, connection rules) |
...
- **Styling**: Ant Design components with custom theming. CSS-in-JS via Ant Design's `styled` API.
