...
- **Component library:** Ant Design + Radix UI primitives
- **State management:** Refine framework (data fetching/routing/CRUD) + custom services (GraphStorageService, WebSocketService)
- **Styling:** Ant Design theming + Tailwind CSS
- **Test runner:** Vitest

## Domain Context

- **Project purpose:** Geniro — AI agent orchestration platform with graph-based visual workflows.
- **Key user flows:** Graph canvas editing (drag-and-drop nodes), thread conversations with AI agents, knowledge base management, repository management.
- **UI conventions:** All UI built from shared component library at `src/components/ui/`. Storybook is source of truth. Socket.IO for real-time updates. Auto-generated API client from OpenAPI spec (`src/autogenerated/`).
- **Domain-specific UI rules:**
  - Never create custom inline components that replicate existing UI primitives.
  - Update the component library first, then use the updated component.
  - Never diverge from storybook visuals — if a page looks different from storybook, the page is wrong.
  - Always regenerate the API client after backend schema changes: `pnpm --filter @geniro/web generate:api`.
...
2. Implement component logic with React 19
3. Add Ant Design theming + Tailwind CSS styling
4. Integrate with Refine or custom services (GraphStorageService, WebSocketService) if needed
...
4. Run `pnpm test:unit` and fix failures
