---
name: plan
description: "Create a detailed implementation plan for a feature or change. Spawns architect-agent, validates with skeptic-agent, saves to .geniro/planning/. Use before /implement or standalone. Do NOT use for trivial 1-2 file changes (/follow-up is better) or when an approved plan already exists."
context: main
model: inherit
allowed-tools:
  - Read
...
| `/plan review` or `/plan list` | List existing plans in `.geniro/planning/` (flat and subdirectories) with status |
