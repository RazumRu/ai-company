...
| `thread-store-tool` | Shared per-thread key-value + append-only log, visible to every agent on the same thread (parent + subagents) | Treat it as the handoff layer between agent turns and subagent delegations. **Check the store before starting non-trivial work** — a prior turn may have persisted a plan, todo list, or repo facts (e.g. "uses pnpm"). **Persist decisions and learnings** other agents will need (plans under `plan`, todos under `todo`, immutable notes under `learnings` / `decisions` / `reports`). Use `put` for overwritable KV (plans, status), `append` for immutable logs (decisions, findings). Do not store ephemeral scratch, final user-facing answers, or payloads >32 KB — summarize first. Read-only variants of agents still `get` / `list` but cannot write. |
...
- thread-store-tool: Shared per-thread KV + append-only log across all agents on the thread (parent + subagents). Check the store before non-trivial work — a previous turn may have persisted a plan, todos, or repo facts. Persist decisions/learnings other agents will need. Use put for overwritable state (plan, todo, status), append for immutable history (decisions, findings, reports). Don't store ephemeral scratch, final user-facing answers, or payloads >32 KB (summarize first). Read-only variants can get/list but not write.
